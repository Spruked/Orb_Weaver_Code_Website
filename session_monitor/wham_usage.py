...
This source is independent from the existing VS Code ``Codex Stats.log``
parser. It reads the local Codex login token from ``~/.codex/auth.json`` (or
``$CODEX_HOME/auth.json``), queries ChatGPT's ``/backend-api/wham/usage``
endpoint, and persists only normalized rate-limit telemetry.

Access tokens, account IDs, email addresses, and raw provider response bodies
are never written to evidence. The request shape was independently implemented
after reviewing the public MIT-licensed ``Maol-1997/codex-stats`` project.

The script runs as a short-lived systemd oneshot. It deliberately does not
instantiate ``Storage`` because a second Storage process would invoke runtime
startup-recovery logic. It reads the active parent runtime ID directly from the
SQLite index and appends through the shared append-only ``EvidenceLog``.
"""
...
def _runtime_data_dir() -> Path:
    ...
    return Path.home() / ".local" / "share" / "code-weaver-runtime"


def load_auth(auth_path: Optional[Path] = None) -> dict:
    """Load request credentials into memory only."""
...
def _reset_minute(window: Optional[dict]):
    if not window:
        return None
    reset_at = _finite_number(window.get("resets_at"))
    return int(reset_at // 60) if reset_at is not None else None


def parse_usage(payload: dict, observed_at: Optional[str] = None) -> dict:
    """Normalize provider data without retaining the raw response body."""
    if not isinstance(payload, dict):
    ...
    state = {
        ...
        "primary_reset_minute": _reset_minute(primary),
        "secondary_reset_minute": _reset_minute(secondary),
    }
    source_record_hash = _canonical_hash(state)

    return {
        "primary_used_percent": state["primary_used_percent"],
        "secondary_used_percent": state["secondary_used_percent"],
        "primary_window_minutes": state["primary_window_minutes"],
        "secondary_window_minutes": state["secondary_window_minutes"],
...
def fetch_usage(
    ...
    """Fetch one provider observation with deliberately coarse failures."""
    headers = {
...
def _latest_stats_quota(evidence: EvidenceLog, session_id: str) -> Optional[dict]:
    for event in reversed(evidence.read_session(session_id)):
        ...
        if isinstance(normalized, dict):
            return {"timestamp": event.get("timestamp"), "normalized": normalized}
    return None
...
def _comparison_payload(stats: Optional[dict], wham: dict) -> dict:
    ...
    age_seconds = (
        abs((wham_time - stats_time).total_seconds())
    ...
    d5 = w5 - s5 if comparable and w5 is not None and s5 is not None else None
    dw = ww - sw if comparable and ww is not None and sw is not None else None
    ...
    if not stats:
        ...
        deltas = [value for value in (d5, dw) if value is not None]
        state = "match" if deltas and all(abs(value) <= 0.01 for value in deltas) else "mismatch"
        if not deltas:
    ...
    return {
        ...
        "delta": {
            "primary_used_percent": d5,
            "secondary_used_percent": dw,
        },
    }


def _record_comparison(evidence: EvidenceLog, session_id: str, wham: dict) -> bool:
    comparison = _comparison_payload(_latest_stats_quota(evidence, session_id), wham)
    fingerprint_basis = {
        "state": comparison["state"],
        "stats_hash": comparison["stats"].get("source_record_hash"),
        "wham_hash": comparison["wham"].get("source_record_hash"),
    ...
    event = EvidenceEvent(
        ...
        data={"normalized": {**comparison, "source_record_hash": fingerprint}},
        timestamp=wham.get("observed_at") or now_iso(),
    )
    return _append_if_changed(evidence, event, fingerprint)
...
def record_once(
    ...
    if auth.get("status") != "ready":
        persisted = _record_unavailable(
            evidence, session_id, str(auth.get("reason") or "auth_unavailable")
        )
        return {
            "status": "unavailable",
    ...
    persisted = _append_if_changed(evidence, event, normalized["source_record_hash"])
    comparison_persisted = _record_comparison(evidence, session_id, normalized)
    return {
        "status": "observed",
        "persisted": persisted,
